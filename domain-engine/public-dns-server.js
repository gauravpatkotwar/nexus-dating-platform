 ...
 * Responds authoritatively to global DNS queries for date.pulse.com
 * with public A-Records and AAAA-Records.
 */
...
server.on('message', (msg, rinfo) => {
  try {
    ...
    // 2. Flags: Standard query response (0x8180), Authoritative Answer (0x8400)
    ...
    // 4. Construct Public A-Record Answer (date.pulse.com -> 49.15.249.115)
    ...
    const answerSection = Buffer.from([
      ...
      0x00, 0x00, 0x0e, 0x10, // TTL: 3600 seconds (1 hour)
    ...
    console.log(`📡 [Public DNS] Resolved ${DOMAIN} -> ${PUBLIC_IPV4} for query from ${rinfo.address}:${rinfo.port}`);
  } catch (err) {
...
server.bind(DNS_PORT, '0.0.0.0', () => {
  console.log(`\n=============================================================`);
  console.log(`📡  AUTHORITATIVE PUBLIC DNS SERVER IS LIVE ON PORT 53!`);
